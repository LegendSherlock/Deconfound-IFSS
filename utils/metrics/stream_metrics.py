...
class StreamSegMetrics(_StreamMetrics):
    ...
    def get_results(self):
        ...
        cls_iu = dict(zip(range(self.n_classes), [iu[i] if m else 0.0 for i, m in enumerate(mask)]))
        cls_acc = dict(
            zip(range(self.n_classes), [acc_cls_c[i] if m else 0.0 for i, m in enumerate(mask)])
        )
        ...
        return {
            ...
            "Mean IoU": mean_iu,
            "Class IoU": cls_iu,
            "Class 0 IoU": float(cls_iu[0]),
            "Class 1 IoU": float(cls_iu[1]),
            "Class 2 IoU": float(cls_iu[2]),
            "Class 3 IoU": float(cls_iu[3]),
            "Class 4 IoU": float(cls_iu[4]),
            "Class 5 IoU": float(cls_iu[5]),
            "Class 6 IoU": float(cls_iu[6]),
            "Class 7 IoU": float(cls_iu[7]),
            "Class 8 IoU": float(cls_iu[8]),
            "Class 9 IoU": float(cls_iu[9]),
            "Class 10 IoU": float(cls_iu[10]),
            "Class 11 IoU": float(cls_iu[11]),
            "Class 12 IoU": float(cls_iu[12]),
            "Class 13 IoU": float(cls_iu[13]),
            "Class 14 IoU": float(cls_iu[14]),
            "Class 15 IoU": float(cls_iu[15]),
            "Class 16 IoU": float(cls_iu[16]),
            "Class 17 IoU": float(cls_iu[17]),
            "Class 18 IoU": float(cls_iu[18]),
            "Class 19 IoU": float(cls_iu[19]),
            "Class 20 IoU": float(cls_iu[20]),
            "Class Acc": cls_acc,
