...
class ResidualBlock(nn.Module):
    ...
    groups : int
        ...
        bottleneck blocks.
    norm : callable
        Function to create normalization Module.
    dropout: callable
    ...
    def __init__(
        ...
        if not is_bottleneck:
            ...
            layers = [
                ...
                ), ("bn1", norm(channels[0])), ("act1", nn.ReLU(inplace=True)),
                (
                    "conv2",
                    nn.Conv2d(
                        channels[0],
                ...
                ), ("bn2", bn2), ("act2", nn.ReLU(inplace=True))
            ]
            ...
            layers = [
                ...
                ), ("bn2", norm(channels[1])), ("act2", nn.ReLU(inplace=True)),
                ("conv3", nn.Conv2d(channels[1], channels[2], 1, stride=1, padding=0, bias=False)),
                ("bn3", bn3), ("act3", nn.ReLU(inplace=True))
            ]
            if dropout is not None:
    ...
    def forward(self, x):
        ...
        act = functional.relu(x, inplace=not self._last)
