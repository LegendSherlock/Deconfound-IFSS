...
class DeeplabV3(nn.Module):
    def __init__(self,
        ...
        self.map_bn = norm(hidden_channels * 4)

        self.global_pooling_conv = nn.Conv2d(in_channels, hidden_channels, 1, bias=False)
        self.global_pooling_bn = norm(hidden_channels)

        self.red_conv = nn.Conv2d(hidden_channels * 4, out_channels, 1, bias=False)
        self.pool_red_conv = nn.Conv2d(hidden_channels, out_channels, 1, bias=False)
        self.red_bn = norm(out_channels)
    ...
    def forward(self, x):
        ...
        if self.training or self.pooling_size is None:
            pool = pool.repeat(1, 1, x.size(2), x.size(3))  # 不是特征双线性上采样?
    ...
    def _global_pooling(self, x):
        if self.training or self.pooling_size is None:
            ...
            pooling_size = (min(self.pooling_size, x.shape[2]),
                            min(self.pooling_size, x.shape[3]))
            padding = (
                (pooling_size[1] - 1) // 2,
