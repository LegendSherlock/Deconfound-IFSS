...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        out_list = []
        act_1, out1 = self.layer1(x)
        act_2, out2 = self.layer2(act_1)
        act_3, out3 = self.layer3(act_2)
        out_list.append(out1)
        out_list.append(out2)
        out_list.append(out3)
        if not self.stop=='layer4':
            act_4, out4 = self.layer4(act_3)
            out_list.append(out4)
            return act_4, out_list

        return act_3, out_list
